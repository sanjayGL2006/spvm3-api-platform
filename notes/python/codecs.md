...
codecs.open(filename, mode='r', encoding=None, errors='strict', buffering=-1)
   ...
   *buffering* has the same meaning as for the built-in "open()"
   ...
   Deprecated since version 3.14: "codecs.open()" has been superseded
   by "open()".
...
class codecs.Codec
   ...
   encode(input, errors='strict')
      ...
      length consumed). For instance, *text encoding* converts a
      string object to a bytes object using a particular character set
   ...
   decode(input, errors='strict')
      ...
      bytes object or one which provides the read-only buffer
...
Changed in version 3.14: On Windows, "cpXXX" codecs are now available
for all code pages.
...
|                      |           | "errors='strict'" is        |
|                      |           | supported.  Warning:   This |
|                      |           | codec builds on "punycode", |
|                      |           | whose algorithms scale      |
|                      |           | poorly, so limit the length |
|                      |           | of untrusted input.         |
+----------------------+-----------+-----------------------------+
...
|                      |           | supported.  Warning:   The  |
|                      |           | decoding and encoding       |
|                      |           | algorithms scale poorly, so |
|                      |           | limit the length of         |
|                      |           | untrusted input.            |
+----------------------+-----------+-----------------------------+
...
Warning:
  ...
  This module builds on "punycode", whose algorithms scale poorly, so

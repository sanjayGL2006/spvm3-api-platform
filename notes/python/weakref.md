...
not in C), instance methods, sets, frozensets, some *file objects*,
*generators*, type objects, sockets, arrays, deques, regular
...
weakref.proxy(object[, callback])
   ...
   nature, and prevents their use as dictionary keys.  *callback* is
...
class weakref.finalize(obj, func, /, *args, **kwargs)
   ...
   When the program exits (or more generally, at *interpreter
   shutdown*), each remaining live finalizer is called unless its
   "atexit" attribute has been set to false. They are called in
   ...
   atexit
      ...
      *interpreter shutdown*, all remaining live finalizers for which
      "atexit" is true are called in reverse order of creation.

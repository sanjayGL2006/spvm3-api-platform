...
environments have their own set of private site packages (i.e. locally
installed libraries), and are optionally segregated from the system-
wide site packages.  Their concept and implementation are inspired by
the popular "virtualenv" third-party package, but benefit from tighter
...
The "array" module supports the long long type using "q" and "Q" type
codes.
...
* The "curses.window" class has a new "get_wch()" method to get a wide
  character

* The "curses" module has a new "unget_wch()" function to push a wide
...
* "format_datetime()": given a "datetime", produce a string formatted
  for use in an email header.

* "parsedate_to_datetime()": given a date string from an email header,
  convert it into an aware "datetime", or a naive "datetime" if the
  offset is "-0000".

* "localtime()": With no argument, returns the current local time as
  an aware "datetime" using the local "timezone".  Given an aware
  "datetime", converts it into an aware "datetime" using the local
  "timezone".
...
* "clock_getres()", "clock_gettime()" and "clock_settime()" functions
  with "CLOCK_*xxx*" constants. (Contributed by Victor Stinner in
  bpo-10278.)
...
* "Py_UNICODE_strcpy()", "Py_UNICODE_strncpy()", "Py_UNICODE_COPY()":
  use "PyUnicode_CopyCharacters()" or "PyUnicode_Substring()"

...
IMAP4.login_cram_md5(user, password)
   ...
   Changed in version 3.14: An "IMAP4.error" is raised if MD5 support
   is not available.
...
IMAP4.capabilities
   ...
   A tuple of the capabilities advertised by the server, in upper
   case.

   It is set when the connection is established, and refreshed after a
   successful "login()", "authenticate()" or "starttls()", because the
   server can advertise different capabilities in different connection
   states.

   Changed in version 3.14.7: Refreshed after "login()" and
   "authenticate()".

...
the value can be interpreted as a truth value (see section Truth Value
Testing above).
...
See Time complexity of operations on built-in types for the costs of
the various sequence operations.
...
sequence.pop(index=-1, /)
   ...
   Retrieve the item at *index* and also remove it from *sequence*. By
...
str.rsplit(sep=None, maxsplit=-1)
   ...
   splitting from the right, "rsplit()" behaves like "split()" which
...
* When the equals sign "'='" is provided in an interpolation
  ...
  explicit conversion or format specifier is provided, this will
...
class bytes(source, encoding, errors='strict')
   ...
   * Copying existing binary data via the buffer protocol:
   ...
   classmethod fromhex(string, /)
      ...
      Changed in version 3.14: "bytes.fromhex()" now accepts ASCII
...
class bytearray(source, encoding, errors='strict')
   ...
   * Copying existing binary data via the buffer protocol:
   ...
   classmethod fromhex(string, /)
      ...
      This "bytearray" class method returns a bytearray object,
      ...
      Changed in version 3.14: "bytearray.fromhex()" now accepts ASCII
      "bytes" and *bytes-like objects* as input.
...
bytearray.strip(bytes=None, /)
   ...
   *bytes* argument defaults to removing "ASCII whitespace". The
   *bytes* argument is not a prefix or suffix; rather, all
...
effect is similar to using the "sprintf()" function in the C language.
...
class memoryview(object)
      ...
      nested list representation of the view. If "view.ndim == 1",
      this is equal to the number of elements in the view.

      Changed in version 3.12: If "view.ndim == 0", "len(view)" now
      raises "TypeError" instead of returning 1.
      ...
      Changed in version 3.4: memoryview is now registered
      automatically with "collections.abc.Sequence"

      Changed in version 3.5: memoryviews can now be indexed with a
      tuple of integers.
...
the "collections" module.) See Time complexity of operations on built-
in types for the costs of the various set operations.
...
See Time complexity of operations on built-in types for the costs of
the various dictionary operations.
...
X | Y | ...
   ...
   Note:
     ...
     The "|" operator cannot be used at runtime to define unions where
...
Changed in version 3.14: Union objects are now instances of

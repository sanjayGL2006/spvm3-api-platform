...
os.getgid()
   ...
   The function is a stub on WASI, see WebAssembly platforms for more
   information.
...
os.fchmod(fd, mode)
   ...
   The function is limited on WASI, see WebAssembly platforms for more
   information.
...
os.readlink(path, *, dir_fd=None)
   ...
   object on Windows.Added support for directory junctions, and
...
os.scandir(path='.')
   ...
   *path* may be a *path-like object*.  If *path* is of type "bytes"
   ...
   Changed in version 3.6: Added support for the *context manager*
...
class os.stat_result
   ...
   Changed in version 3.7: Added the "st_fstype" member to
   Solaris/derivatives.

   Changed in version 3.8: Added the "st_reparse_tag" member on
   Windows.
...
os.timerfd_create(clockid, /, *, flags=0)
   ...
   * "time.CLOCK_BOOTTIME" (Since Linux 3.15 for timerfd_create)
   ...
   time clock is used. If the system clock is changed, the timer
   setting needs to be updated. To cancel the timer when the system
   clock is changed, see "TFD_TIMER_CANCEL_ON_SET".
   ...
   * "TFD_CLOEXEC"
   ...
   "read()" raises "OSError" with "errno" set to "errno.EAGAIN".
...
os.timerfd_settime(fd, /, *, flags=0, initial=0.0, interval=0.0)
   ...
   *initial* is greater than zero, the timer is enabled. If *initial*
   is less than zero, it raises an "OSError" exception with "errno"
   set to "errno.EINVAL".
   ...
   *interval* is zero, the timer only fires once, on the initial
   ...
   is aborted with the error "errno.ECANCELED".
...
os.cpu_count()
   ...
   Changed in version 3.13: If "-X cpu_count" is given or

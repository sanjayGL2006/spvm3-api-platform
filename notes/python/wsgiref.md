...
"WSGIEnvironment" for a type alias that can be used in type
annotations.
...
class wsgiref.util.FileWrapper(filelike, blksize=8192)
   ...
   protocol used to convert a file-like object to an *iterator*. The
   resulting objects are *iterable*s. As the object is iterated over,
   the optional *blksize* parameter will be repeatedly passed to the
...
class wsgiref.simple_server.WSGIRequestHandler(request, client_address, server)
   ...
   *handler_class* to the "make_server()" function.  Some possibly
   ...
   get_environ()
      ...
      Return a "WSGIEnvironment" dictionary for a request.  The
      default implementation copies the contents of the "WSGIServer"
      object's "base_environ" dictionary attribute and then adds
      various headers derived from the HTTP request.  Each call to
      this method should return a new dictionary containing all of the
...
class wsgiref.handlers.BaseHandler
   ...
   get_stdin()
      ...
      Return an object compatible with "InputStream" suitable for use
      as the "wsgi.input" of the request currently being processed.

   get_stderr()

      Return an object compatible with "ErrorStream" suitable for use
      as the "wsgi.errors" of the request currently being processed.
   ...
   wsgi_file_wrapper
      ...
      "wsgiref.types.FileWrapper", or "None".  The default value of

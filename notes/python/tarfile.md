...
Changed in version 3.12: Archives are extracted using a filter, which
makes it possible to either limit surprising/dangerous features, or to
acknowledge that they are expected and the archive is fully trusted.

Changed in version 3.14: Set the default extraction filter to "data",
which disallows some dangerous features such as links to absolute
paths or paths outside of the destination. Previously, the filter
strategy was equivalent to "fully_trusted".

Changed in version 3.14: Added support for Zstandard compression using
"compression.zstd".
...
tarfile.open(name=None, mode='r', fileobj=None, bufsize=10240, **kwargs)
   ...
   *zstd_dict* can be passed to provide a "ZstdDict", a Zstandard
   ...
   it does not allow random access, see Examples.  The currently
   possible modes:
   ...
   Changed in version 3.14: The *preset* keyword argument also works
   for streams.
...
TarFile.extractall(path='.', members=None, *, numeric_owner=False, filter=None)
   ...
   Changed in version 3.14: The *filter* parameter now defaults to
   "'data'".
...
TarFile.extractfile(member)
   ...
   Changed in version 3.13: The returned "io.BufferedReader" object
...
TarFile.extraction_filter
   ...
   Changed in version 3.14: The default filter is set to "data", which
...
TarInfo.mtime: int | float
   ...
   Changed in version 3.12: Can be set to "None" for "extract()" and
...
* "None" (default): Use "TarFile.extraction_filter".
  ...
  used.
     ...
     Changed in version 3.14: The default filter is set to "data".
...
tarfile.tar_filter(member, path)
   ...
   * Normalize filenames ("TarInfo.name") that contain ".." components
     using "os.path.normpath()". Note that this removes internal ".."
     components, which may change the meaning of the name if it
   ...
   * Clear high mode bits (setuid, setgid, sticky) and group/other
   ...
   Changed in version 3.14.7 (unreleased): Filenames containing ".."

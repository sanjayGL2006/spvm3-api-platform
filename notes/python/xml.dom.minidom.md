...
* Each of the "NodeList" and "NamedNodeMap" interfaces has two
  implementations, which provide additional methods and operations.

  "childNodes" is a subclass of "list", or, for nodes which cannot
  have children, a subclass of "tuple". It supports iteration,
  concatenation, indexing and slicing.

  "attributes" supports "len()", the "in" operator, subscription by a
  name or by a "(namespaceURI, localName)" tuple, assignment and
  deletion, and the methods "get()", "keys()", "keysNS()", "values()",
  "items()" and "itemsNS()". "entities" and "notations" are read-only
  and support only "len()" and subscription by a name.

* "strictErrorChecking" and "specified" are always "False".

* "removeAttribute()" and "removeAttributeNS()" raise "NotFoundErr" if
  there is no matching attribute, while the DOM specifies that this
  has no effect.

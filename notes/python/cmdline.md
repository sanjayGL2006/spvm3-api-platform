...
-c <command>
   ...
   Changed in version 3.14: *command* is automatically dedented before
   execution.
...
-b
   ...
   Changed in version 3.5: Affects also comparisons of "bytes" with
   "int".
...
-R
   ...
   complexity.  See https://ocert.org/advisories/ocert-2011-003.html
   for details.
...
-x
   ...
   Similarly to adding a shebang line and setting the executable bit
   on Unix, the extension of the Python script can be changed to
   ".bat" and the following line can be added at the start of the
   script:
   ...
   Unlike a shebang line which is a Python comment, this line is not
   valid Python syntax, and the "-x" option is needed to skip it.
...
-X
   ...
   * "-X importtime" to show how long each import takes. It shows
     ...
     Changed in version 3.14: Added "-X importtime=2" to also trace
...
PYTHONPROFILEIMPORTTIME
   ...
   Changed in version 3.14: Added "PYTHONPROFILEIMPORTTIME=2" to also
...
PYTHONMALLOC
   ...
   * "mimalloc_debug": same as "mimalloc" but also install debug
   ...
   Note:
     ...
     In the *free-threaded* build, the "malloc", "malloc_debug",
     "pymalloc", and "pymalloc_debug" values are not supported.  Only
     "default", "debug", "mimalloc", and "mimalloc_debug" are
     accepted.
...
PYTHONMALLOCSTATS
   ...
   pymalloc memory allocator or the mimalloc memory allocator
   (whichever is in use) every time a new object arena is created, and
   on shutdown.
   ...
   Python is configured without both "pymalloc" and "mimalloc"
   support.

...
* Setting a header on a "Message" results in that header being parsed
  and a header object created.
...
* Any header object, or any header that is refolded due to the policy
  ...
  folding algorithms, including knowing where encoded words are
...
string value is the fully decoded value of the header.  Likewise, a
header may be assigned a new value, or a new header created, using a
string, and the policy will take care of converting the string into

...
time.asctime([time_tuple])
   ...
   and is space padded if the day is a single digit, for example:
...
time.ctime(seconds=None, /)
   ...
   a single digit, for example: "'Wed Jun  9 04:26:40 1993'".
...
time.gmtime(seconds=None, /)
   ...
   *seconds* is not provided or "None", the current time as returned
   by "time()" is used.  Fractions of a second are ignored.  See above
...
time.localtime(seconds=None, /)
   ...
   Like "gmtime()" but converts to local time. If *seconds* is not
...
time.perf_counter() -> float
   ...
   **CPython implementation detail:** On CPython, use the same clock
   as "time.monotonic()" and is a monotonic clock, i.e. a clock that
   cannot go backwards.
...
time.sleep(seconds, /)
   ...
   On Windows, if *seconds* is zero, the thread relinquishes the
   ...
   returns immediately, and the thread continues execution. On Windows
   10 and newer the implementation uses a high-resolution timer which
   provides resolution of 100 nanoseconds. If *seconds* is zero,
   "Sleep(0)" is used.
   ...
   * Or use "select()" (resolution: 1 microsecond).
   ...
   Changed in version 3.5: The function now sleeps at least *seconds*
...
time.strftime(format[, time_tuple])
   ...
   *format* argument.  If *time_tuple* is not provided, the current
   time as returned by "localtime()" is used.  *format* must be a
   string.  "ValueError" is raised if any field in *time_tuple* is
   ...
   4. When used with the "strptime()" function, "%U" and "%W" are only

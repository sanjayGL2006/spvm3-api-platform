...
exhausted
   An *iterator* that has produced all of its values is said to be
   *exhausted*. Further attempts to get the next value (for example,
   calls to "next()") raise "StopIteration" (or "StopAsyncIteration"
   in the case of an *asynchronous iterator*).
...
expression
   ...
   which all return a value.  Not all language constructs are
   expressions.  There are also *statement*s which cannot be used as
   expressions, such as "while".  Assignments are also statements, not
   expressions.
...
generator function
   ...
   retrieved one at a time with the "next()" function. See Yield
   expressions.
...
generator iterator
   ...
   See Generator-iterator methods.
...
iterator
   ...
   **CPython implementation detail:** CPython does not consistently
   ...
   also please note that *free-threaded* CPython does not guarantee
...
list
   ...
   elements is *O*(1).  See Time complexity of operations on built-in
   types.

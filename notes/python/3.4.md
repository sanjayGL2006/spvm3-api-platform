...
* Running IDLE --- Python editor and shell with the "-n" flag (no
  subprocess) is deprecated. However, the feature will not be removed
  until bpo-18823 is resolved.
...
* Because "unittest.TestSuite" now drops references to tests after
  they are run, test harnesses that reuse a "TestSuite" to re-run a
